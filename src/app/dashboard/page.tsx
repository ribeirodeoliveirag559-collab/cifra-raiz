              ...
              {emAndamento.length > 0 && (
                <section>
                  ...
                  <div className="space-y-3">
                    {emAndamento.map((m) => (
                      <div key={m.id} className="relative bg-white rounded-2xl border border-[#F0EAE0] p-5 overflow-hidden">
                        ...
                        <div className="h-2 bg-[#F0EAE0] rounded-full overflow-hidden">
                          <div className="h-full bg-[#D4900A] rounded-full" style={{ width: `${m.pct}%` }} />
                        </div>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-2xl select-none"
                          ...
                          <div className="animate-respirar flex flex-col items-center gap-2">
                            <span className="text-2xl font-display font-bold text-white drop-shadow-lg">Em Breve!</span>
                            <span className="text-[#D4900A] text-xs font-semibold tracking-widest uppercase">Videoaulas em produção</span>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {naoIniciados.slice(0, 4).map((m) => (
                      <div key={m.id} className="relative bg-white rounded-xl border border-[#F0EAE0] p-4 overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl select-none"
                          style={{ background: "rgba(0,0,0,0.65)" }}>
                          <div className="animate-respirar flex flex-col items-center gap-2">
                            <span className="text-xl font-display font-bold text-white drop-shadow-lg">Em Breve!</span>
                            <span className="text-[#D4900A] text-xs font-semibold tracking-widest uppercase">Em produção</span>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {concluidos.length > 0 && (
                <section>
                  ...
                  <div className="space-y-2">
                    {concluidos.map((m) => (
                      <div key={m.id} className="relative flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3 overflow-hidden">
