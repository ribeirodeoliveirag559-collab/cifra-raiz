      ...
      <div className="max-w-6xl mx-auto px-4 h-14 md:h-16 flex items-center justify-between gap-3">
        ...
        {user && (
          <div className="md:hidden flex items-center gap-3 shrink-0">
            <span className="text-sm text-[#FAF7F2] font-semibold truncate max-w-[80px]">
              {user.nome.split(" ")[0]}
            </span>
            <button
              onClick={handleLogout}
              className="text-xs text-[#B5865A] hover:text-[#D4900A] border border-[#7A4520] px-3 py-1.5 rounded-full transition-colors shrink-0"
            >
              Sair
            </button>
          </div>
